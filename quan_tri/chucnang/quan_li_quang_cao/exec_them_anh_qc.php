<?php
	...
	if(isset($_FILES['image'])){
		...
		if($error== 4){
				echo "<script> alert('Không có file nào được chọn'); location.href='?menu=them_anh_qc'; </script>";
			}
			else{
			if(file_exists($img_location)){
				echo "<script> alert('Ảnh đã tồn tại'); location.href='?menu=them_anh_qc'; </script>";
			}
			else
			{
	...
	$note= $_POST['note'];
	$sql= "insert into slideshow(name,note,loai) values(?,?,'qc')";
	$stmt= $db->prepare($sql);
	$stmt->bindParam(1,$image);
	$stmt->bindParam(2,$note);
	if($stmt->execute()){
		echo "<script> alert('Thêm ảnh thành công'); location.href='?menu=quanli_quang_cao'; </script>";
	}
	else{
		unlink('../images/slideshow'.$image);
		echo "<script> alert('Thêm ảnh không thành công'); location.href='?menu=quanli_quang_cao'; </script>";
	}
}
}
